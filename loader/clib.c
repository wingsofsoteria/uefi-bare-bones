...
static void buffer_append(struct format_buffer *buffer, int code) {
  if (buffer->count < buffer->size) buffer->buffer[buffer->count] = code;
  ++buffer->count;
}

static void buffer_copy_u16(struct format_buffer *buffer, const uint16_t *begin,
                            const uint16_t *end) {
  const size_t len = end - begin;
  const size_t rem =
      buffer->count < buffer->size ? buffer->size - buffer->count : 0;
  const size_t copy = len < rem ? len : rem;
  uint16_t *pos = buffer->buffer + buffer->count;

  for (size_t i = 0; i < copy; ++i) *pos++ = begin[i];
  buffer->count += len;
}

static void buffer_copy(struct format_buffer *buffer, const char *begin,
                        const char *end) {
  const size_t len = end - begin;
  const size_t rem =
      buffer->count < buffer->size ? buffer->size - buffer->count : 0;
...
static void buffer_format_unsigned(struct format_buffer *buffer,
...
static void buffer_format_signed(struct format_buffer *buffer,
                                 struct format *config, long long v) {
  if (v < 0) {
...
static const char *format_parse_type_width(const char *str,
                                           struct format *config) {
  if (*str == 'z') {
...
static enum format_type format_adjusted_type(enum format_type type,
                                             enum format_type_width width) {
  if (type == FMT_INT) {
    switch (width) {
      case FMT_TYPE_CHAR:
        return FMT_SCHAR;
...
int vsnprintf(uint16_t *buffer, size_t size, const char *fmt, va_list args) {
  ...
  while (*current) {
    ...
    if (config.width_from_args) config.width = (unsigned)va_arg(args, int);
...
void *memset(void *ptr, int value, size_t size) {
  ...
  for (size_t i = 0; i < size; ++i) *to++ = value;
  return ptr;
}
...
void *memcpy(void *dst, const void *src, size_t size) {
  ...
  for (size_t i = 0; i < size; ++i) *to++ = *from++;
  return dst;
}
