...
static efi_status_t add_module(struct loader *loader, const char *name,
                               const uint16_t *path) {
  if (loader->modules == loader->module_capacity) {
    ...
    status = loader->system->boot->allocate_pool(
        EFI_LOADER_DATA, new_size * sizeof(struct module),
        (void **)&new_module);
    if (status != EFI_SUCCESS) {
      err(loader->system, "failed to allocate buffer for modules\r\n");
      return status;
    }

    memcpy(new_module, old_module, loader->modules * sizeof(struct module));
    loader->module = new_module;
    loader->module_capacity = new_size;

    if (old_module != NULL) {
      status = loader->system->boot->free_pool((void *)old_module);
      if (status != EFI_SUCCESS) {
        err(loader->system, "failed to free buffer for modules\r\n");
        return status;
      }
    }
  }
...
efi_status_t load_config(struct loader *loader, const uint16_t *config_path) {
  ...
  status = loader->rootdir->open(loader->rootdir, &loader->config,
                                 (uint16_t *)config_path, EFI_FILE_MODE_READ,
                                 EFI_FILE_READ_ONLY);
  if (status != EFI_SUCCESS) {
    err(loader->system, "failed to open %w: %llu\r\n", config_path,
        (unsigned long long)status);
    return status;
  }

  size = sizeof(file_info);
  status = loader->config->get_info(loader->config, &guid, &size,
                                    (void *)&file_info);
  if (status != EFI_SUCCESS) {
    err(loader->system, "failed to find size of %w: %llu\r\n", config_path,
        (unsigned long long)status);
    return status;
  }

  status = loader->system->boot->allocate_pool(
      EFI_LOADER_DATA, file_info.file_size + 1, (void **)&loader->config_data);
  if (status != EFI_SUCCESS) {
    err(loader->system, "failed to allocate buffer for %w data: %llu\r\n",
        config_path, (unsigned long long)status);
    return status;
  }

  memset((void *)loader->config_data, 0, file_info.file_size + 1);

  return efi_read_fixed(loader->system, loader->config,
                        /* offset */ 0,
...
efi_status_t parse_config(struct loader *loader) {
  ...
  while (1) {
    ...
    if (loader->config_data[i] == '\0') break;
    ...
    if (loader->config_data[i] != ':') {
      err(loader->system, "incorrect config format: missing ':'\r\n");
    ...
    if (name_size == 0) {
      err(loader->system, "invalid config format: empty module name\r\n");
      return EFI_INVALID_PARAMETER;
    }

    if (path_size == 0) {
      err(loader->system, "invalid config format: empty module path\r\n");
      return EFI_INVALID_PARAMETER;
    }

    status = loader->system->boot->allocate_pool(EFI_LOADER_DATA, name_size + 1,
                                                 (void **)&name);
    if (status != EFI_SUCCESS) {
      err(loader->system, "failed to allocate buffer for module name\r\n");
      return status;
    }
    ...
    status = loader->system->boot->allocate_pool(
        EFI_LOADER_DATA, 2 * (path_size + 1), (void **)&path);
    if (status != EFI_SUCCESS) {
      err(loader->system, "failed to allocate buffer for module path\r\n");
      return status;
    }
    to_u16strncpy(path, &loader->config_data[path_begin], path_size);
    path[path_size] = '\0';

    status = add_module(loader, name, path);
    if (status != EFI_SUCCESS) {
      err(loader->system, "failed to add module %s\r\n", name);
      return status;
    }
  }

  for (size_t j = 0; j < loader->modules; ++j) {
    if (strcmp(loader->module[j].name, "kernel") != 0) continue;
  ...
  err(loader->system, "invalid config format: no kernel module\r\n");
  return EFI_INVALID_PARAMETER;
}
