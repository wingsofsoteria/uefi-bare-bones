...
uint64_t virtual_to_physical(uint64_t virtual)
{
  ...
  {
    kernel_log_error(
      "Could not get level 3 page table, %x is not mapped to any physical address",
      virtual);
    return 0;
  }
  ...
  {
    kernel_log_error(
      "Could not get level 2 page table, %x is not mapped to any physical address",
      virtual);
    return 0;
  }
  ...
  {
    kernel_log_error(
      "Could not get level 1 page table, %x is not mapped to any physical address",
      virtual);
    return 0;
  }
  ...
  {
    kernel_log_error(
      "Could not get page entry, %x is not mapped to any physical address",
      virtual);
    return 0;
  }
...
static void __allocate_entry(
  page_table_t* table, uint16_t index, uint16_t flags)
{
  uint64_t address = allocate_frame();
  if (address == 0)
  {
    abort_msg("Failed to allocate space for page entry");
  }
  set_entry(&table->pages[index], address, flags);
}

void __map_page(uint64_t page, uint64_t frame, uint16_t flags)
{
  kernel_log_debug("Attempting to map frame %x to page %x", frame, page);
  ...
  kernel_log_debug("Got level 4 table at %x", page_table);
  ...
  kernel_log_debug("Got level 3 table at %d: %x", p4_index, p3);
  ...
  kernel_log_debug("Got level 2 table at %d: %x", p3_index, p2);
  ...
  kernel_log_debug("Got level 1 table at %d: %x", p2_index, p1);
  kernel_log_debug("Got page entry at %d", p1_index);
  set_entry(&p1->pages[p1_index], frame, flags);
  flush_tlb(page);
  kernel_log_debug("Flushed TLB");
}
