...
uint64_t virtual_to_physical(uint64_t virtual)
{
  ...
  {
    kernel_log_error(
      "Could not get level 3 page table, %lu is not mapped to any physical address",
      virtual);
    return 0;
  }
  ...
  {
    kernel_log_error(
      "Could not get level 2 page table, %lu is not mapped to any physical address",
      virtual);
    return 0;
  }
  ...
  {
    kernel_log_error(
      "Could not get level 1 page table, %lu is not mapped to any physical address",
      virtual);
    return 0;
  }
...
void map_page(uint64_t page, uint64_t frame, uint16_t flags)
{
  ...
  if (debug)
  {
    kernel_log_debug(
      ...
      frame, page, p4_index, p3, p3_index, p2, p2_index, p1, p1_index);
  }
