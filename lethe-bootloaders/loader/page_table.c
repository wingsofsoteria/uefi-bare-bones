...
void map_pages(void* v_addr, void* p_addr, uint64_t pages, uint16_t flags)
{
  for (int j = 0; j < pages; j++)
    {
      if ((uint64_t)v_addr % EFI_PAGE_SIZE != 0) { return; }
      else if ((uint64_t)p_addr % EFI_PAGE_SIZE != 0) { return; }
...
void copy_existing_pages()
{
  ...
  asm volatile("mov %%cr3, %0"
    : "=r"(cr3));
  for (uint64_t i = 0; i < 512; i++) { p4_table->pages[i] = cr3->pages[i]; }
}
  ...
  while (1)
    {
      ...
      if (status == EFI_SUCCESS)
        return (mmap_t){ (loader_memory_descriptor_t*)memory_map,
          mmap_size,
          desc_size,
          desc_version,
          map_key };
