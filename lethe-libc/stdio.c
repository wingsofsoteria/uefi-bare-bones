...
static size_t print_reverse(const char *buf, size_t len, uint32_t flags,
                            uint32_t width, size_t idx) {
...
static size_t __ntoa(char *buf, size_t len, uint32_t flags, uint32_t width,
                     uint32_t precision, uint32_t base, bool negative,
                     size_t idx) {
  if (!(flags & FLAGS_LEFT)) {
...
static size_t ntoa_long(unsigned long value, uint32_t flags, uint32_t base,
                        uint32_t width, uint32_t precision, size_t idx,
                        bool negative) {
  char buf[NTOA_MAX];
  size_t len = 0;
  if (!value)
...
static size_t ntoa_long_long(unsigned long long value, uint32_t flags,
                             uint32_t base, uint32_t width, uint32_t precision,
                             size_t idx, bool negative) {
  char buf[NTOA_MAX];
  size_t len = 0;
  if (!value)
    flags &= ~FLAGS_HASH;
  ...
  while (*fmt) {
    ...
    case 'b': {
      ...
      if ((*fmt == 'i') || (*fmt == 'd')) {
        if (flags & FLAGS_LONG_LONG) {
          ...
          idx = ntoa_long_long(
              (unsigned long long)(value > 0 ? value : 0 - value), flags, base,
              width, precision, idx, value < 0);
    ...
    case 'p': {
      ...
      if (sizeof(uintptr_t) == sizeof(long long)) {
        ...
        idx = ntoa_long((unsigned long)((uintptr_t)va_arg(va, void *)), flags,
                        16, width, precision, idx, false);
      }
      fmt++;
      break;
    }
