...
#include <stdint.h>
uint8_t next_byte();
uint8_t peek_byte(int);
// aml_ptr_t one_of(void*, int, ...);
aml_ptr_t parse_term_list(void*, int);
aml_ptr_t parse_name_string();
aml_ptr_t parse_namespace_modifier_obj(void*);
aml_ptr_t parse_named_obj(void*);
