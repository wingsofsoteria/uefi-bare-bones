...
{
  return print(string, strlen(string));
}
...
static uint32_t atoi(const char** a)
{
  ...
  while (is_digit(**a))
  {
    i = (i * 10) + (uint32_t)((*(*a)++) - 0x30);
  }
  return i;
}

static size_t print_reverse(
  const char* buf, size_t len, uint32_t flags, uint32_t width, size_t idx)
{
...
static size_t __ntoa(char* buf, size_t len, uint32_t flags, uint32_t width,
  uint32_t precision, uint32_t base, bool negative, size_t idx)
{
  if (!(flags & FLAGS_LEFT))
  {
    ...
    while (len < precision && len < NTOA_MAX)
    {
      buf[len++] = '0';
    }
...
static size_t ntoa_long(unsigned long value, uint32_t flags, uint32_t base,
  uint32_t width, uint32_t precision, size_t idx, bool negative)
{
  char buf[NTOA_MAX];
  size_t len = 0;
  if (!value)
...
static size_t ntoa_long_long(unsigned long long value, uint32_t flags,
  uint32_t base, uint32_t width, uint32_t precision, size_t idx, bool negative)
{
  char buf[NTOA_MAX];
  size_t len = 0;
  if (!value)
  ...
  while (*fmt)
  {
    ...
    if (*fmt == '.')
    {
      ...
      else if (*fmt == '*')
      {
        ...
        precision = precision_specifier > 0 ? (uint32_t)precision_specifier : 0;
        fmt++;
      }
    }
    switch (*fmt)
      ...
      case 'b':
        {
          ...
          if ((*fmt == 'i') || (*fmt == 'd'))
          {
            if (flags & FLAGS_LONG_LONG)
            {
              ...
              idx                   = ntoa_long_long(
                (unsigned long long)(value > 0 ? value : 0 - value), flags,
                base, width, precision, idx, value < 0);
            }
            else if (flags & FLAGS_LONG)
            {
              const long value = va_arg(va, long);
              idx = ntoa_long(value > 0 ? value : 0 - value, flags, base, width,
                precision, idx, value < 0);
            }
            else
            {
              int value = 0;
          ...
          else
          {
            if (flags & FLAGS_LONG_LONG)
            {
              idx = ntoa_long_long(va_arg(va, unsigned long long), flags, base,
                width, precision, idx, false);
            }
            else if (flags & FLAGS_LONG)
            {
              idx = ntoa_long(va_arg(va, unsigned long), flags, base, width,
                precision, idx, false);
            }
            else
            {
              ...
              idx = ntoa_long(value, flags, base, width, precision, idx, false);
            }
          }
          fmt++;
          break;
      ...
      case 'p':
        {
          ...
          {
            idx = ntoa_long_long((uintptr_t)va_arg(va, void*), flags, 16, width,
              precision, idx, false);
          }
          else
          {
            idx = ntoa_long((unsigned long)((uintptr_t)va_arg(va, void*)),
              flags, 16, width, precision, idx, false);
          }
          fmt++;
          break;
        }
