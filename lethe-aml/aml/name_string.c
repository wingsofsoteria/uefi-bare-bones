...
{
  if (
    LEAD_CHAR_OOB(peek_byte(0)) || NAME_CHAR_OOB(peek_byte(1)) ||
  ...
  for (int i = 0; i < 4; i++) { segment[i] = next_byte(); }
  segment[4] = 0;
  return segment;
}
...
{
  uint8_t token = next_byte();
  if (token == NULL_NAME) { return (aml_ptr_t){ NULL_NAME, NULL }; }
...
aml_ptr_t parse_name_string()
{
  ...
  while (token == PREFIX_CHAR) { token = next_byte(); }
  aml_ptr_t name_path = parse_name_path();
  AML_ERR_CHECK(name_path);
