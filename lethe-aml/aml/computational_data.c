  ...
  switch (token)
    {
      case EXT_OP_PREFIX:
        {
          token = next_byte();
          if (token != EXT_REVISION_OP) { return AML_PREFIX_ERROR; }
          return (aml_ptr_t){ EXT_REVISION_OP, NULL };
        }
      case BYTE_PREFIX: return byte_data();
      case WORD_PREFIX: return word_data();
      case DWORD_PREFIX: return dword_data();
      case QWORD_PREFIX: return qword_data();
      case STRING_PREFIX: return parse_string();
      case ZERO_OP:
      case ONE_OP:
      case ONES_OP: return (aml_ptr_t){ token, NULL };
