...
__inline static unsigned int __get_cpuid_max(
  unsigned int  __leaf,
  unsigned int* __sig
)
{
...
///    returns 0.
__inline static int __get_cpuid(
  unsigned int  __leaf,
  unsigned int* __eax,
  unsigned int* __ebx,
  unsigned int* __ecx,
...
/// \returns Returns 1 if the requested \c CPUID leaf is supported; otherwise
///    returns 0.
__inline static int __get_cpuid_count(
  unsigned int  __leaf,
...
// -fms-extensions) which will conflict with the __cpuidex definition below.
  ...
  // In some cases, offloading will set the host as the aux triple and define
  // the builtin. Given __has_builtin does not detect builtins on aux triples,
  // we need to explicitly check for some offloading cases.
...
///    "subleaf") at which the \c CPUID instruction will be executed.
__inline static void __cpuidex(int __cpu_info[4], int __leaf, int __subleaf)
{
  __cpuid_count(
    __leaf,
    __subleaf,
