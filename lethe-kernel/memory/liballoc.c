 ...
 */
inline static int getexp(unsigned int size)
{
  if (size < (1 << MINEXP))
    {
#ifdef DEBUG
...
inline static void insert_tag(struct boundary_tag* tag, int index)
{
  int realIndex;
...
inline static struct boundary_tag* melt_left(struct boundary_tag* tag)
{
...
inline static struct boundary_tag* absorb_right(struct boundary_tag* tag)
{
...
#ifdef DEBUG
  printf(
    "Resource allocated %x of %i pages (%i bytes) for %i size.\n",
    tag,
    pages,
    pages * l_pageSize,
    size
...
void* malloc(size_t size)
{
      ...
      if (childIndex >= 0)
        {
          ...
          new_tag = new_tag; // Get around the compiler warning about unused
                             // variables.

#ifdef DEBUG
          printf(
            "Old tag has become %i bytes, new tag is now %i bytes (%i exp)\n",
            tag->real_size,
            new_tag->real_size,
            new_tag->index
          );
  ...
  ptr = (void*)((unsigned char*)tag + sizeof(struct boundary_tag));
...
void free(void* ptr)
{
  ...
  tag =
    (struct boundary_tag*)((unsigned char*)ptr - sizeof(struct boundary_tag));
...
#ifdef DEBUG
      printf(
        "Melting tag left into available memory. Left was %i, becomes %i "
        "(%i)\n",
