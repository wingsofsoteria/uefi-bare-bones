...
static void debug()
{
  ...
  while (block < allocator.tail && block->size > 0) {
    kernel_log_debug("%p %zu %s", block, block->size,
                     block->free ? "free" : "used");
    block = next_block(block);
  }
}

static struct AllocatorBlock* split_block(struct AllocatorBlock* block,
                                          size_t                 size)
{
...
static bool can_merge(struct AllocatorBlock* block,
                      struct AllocatorBlock* buddy)
{
