...
uint64_t             virtual_to_physical(uint64_t virtual)
{
  ...
  if (p3_physical == 0) {
    kernel_log_error("Could not get level 3 page table, %lu is not mapped to "
  ...
  if (p2_physical == 0) {
    kernel_log_error("Could not get level 2 page table, %lu is not mapped to "
                     "any physical address",
                     virtual);
    return 0;
  }
  ...
  if (p1_physical == 0) {
    kernel_log_error("Could not get level 1 page table, %lu is not mapped to "
                     "any physical address",
                     virtual);
    return 0;
  }
  ...
  if (page_entry == 0) {
    kernel_log_error(
        "Could not get page entry, %lu is not mapped to any physical address",
        virtual);
    return 0;
  }
...
static void __allocate_entry(page_table_t* table, uint16_t index,
                             uint16_t flags)
{
...
void map_page(uint64_t page, uint64_t frame, uint16_t flags)
{
  ...
  if (debug) {
    kernel_log_debug("Mapping address %lu to %lu\n\tP4[%i]: %p\n\tP3[%i]: "
                     "%p\n\tP2[%i]: %p\n\tEntry: %i",
                     frame, page, p4_index, p3, p3_index, p2, p2_index, p1,
                     p1_index);
  }
...
void unmap_page(uint64_t page)
{
  ...
  page_table_t* p1 =
      (void*)((p2->pages[p2_index].address << 12) + hhdm_mapping);
  if (debug) {
    kernel_log_debug("Unmapping page %lu\n\tP4[%i]: %p\n\tP3[%i]: "
                     "%p\n\tP2[%i]: %p\n\tEntry: %i",
                     page, p4_index, p3, p3_index, p2, p2_index, p1, p1_index);
  }
